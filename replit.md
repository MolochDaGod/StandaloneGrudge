...
- **Z-Index Layer System (`src/constants/layers.js`):** Two-context architecture. The content wrapper in App.jsx sits at z-index 10501 (above frame border at 10500), creating a stacking context. All screen-internal layers (map, battle, scene) use low relative values within that context. Overlay siblings (Settings, LootPopup, AdminGizmo) render outside the wrapper at elevated z-indices (10510+). INSIDE content wrapper: BACKGROUND (0) → TERRAIN (1-4) → NODES (5-10) → HUD (12-16) → DEV (30-40) → POPUPS (50-100) → SCENE (5-50) → BATTLE (0-500) → CHAT_BUBBLES (9500) → BOTTOM_BAR (10600). OUTSIDE content wrapper: LOADING (10505) → SETTINGS (10510+) → ADMIN (10515+) → LOOT (10520+) → INTRO (10800) → TOOLTIP (99999, portaled to body). Legacy MAP_LAYERS compatibility object also exported for WorldMap.
- **Game UI Overlay System (`#game-ui-overlay`):** Follows industry-standard game UI overlay architecture (MMO/RTS/RPG pattern). A full-screen transparent layer (`pointer-events: none`, z-index 10600) covers the game, with only interactive panels receiving clicks via `.ui-element` class (`pointer-events: auto`). Three bottom-aligned panels use `.panel-style` class (dark gradient background, gold borders, corner rivets, inset shadows). CSS classes: `.panel-style` (stone/gold panels), `.hotbar-slot` (48px action slots with hover glow), `.circle-btn` (round toggle buttons with active state). All defined in `index.css`.
- **Map Bottom Bar (MapBottomBar.jsx):** Renders inside `#game-ui-overlay`. Three panels: left (Party Log chat, 260px, `.panel-style`), center (8 hotbar action slots in `.panel-style`, max 520px), right (War Party status, 200px, `.panel-style` with 3 `.circle-btn` popups above). Popups (Harvest, Gear, Character) use `.panel-style` and absolute positioning.
