...
- **Hero Codex:** Static HTML page at `public/hero-codex.html` — complete visual reference for all 25 playable heroes. Features treasure room banner image, "How Heroes Work" info section, stat reference (HP/ATK/DEF/SPD/RNG/MP), class archetypes, race stat modifiers grid, faction cards, secret hero reveal, and full hero card grid with race/class/faction/rarity filtering + search. Each hero card shows race icon, name, title, tags, stat bars, and signature quote. Route added to `server.js` htmlPages array.
