...
- **Sprite System:** `SpriteAnimation` component for sprite sheet animations with dynamic scaling, transformations, hue overlays for equipment, and fallback animations. All sprites are scaled by frameHeight to a uniform 200px base height, then multiplied by `getRaceHeightScale(raceId)` for lore-accurate proportional heights (dwarf 5ft/167px, human/undead 6ft/200px, elf/orc 6'4"/211px, barbarian/worge-bear 7ft/233px). No per-combo scale overrides.
