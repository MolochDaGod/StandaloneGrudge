...
**UI/UX Decisions:**
- **Typography & Visuals:** Uses Cinzel (headings), Jost (body), and LifeCraft (Warcraft-style, `.font-warcraft`) fonts. LifeCraft loaded via CDNFonts CDN. Visuals include pixel art sprites, particle and beam effects, a 2D world map with clickable nodes, and animated hero movement. Character cards use race-specific background images.
...
- **Sprite System:** Employs a `SpriteAnimation` component for sprite sheet animations, with dynamic scaling, transformations, per-slot equipment hue overlays, and `will-change: background-position` optimization. Supports fallback animations via `fallbackAnim` property in ability effect maps. Specific enemy and elite warrior sprites are implemented. Full animation set per sprite includes: idle, walk, run, attack1-3, runAttack, charge, roll, jump, fall, slide, surf, fly, block, cast, heal, hurt, death, projectile.
- **Hero Slideshow (LobbyScreen):** "Opening ceremony" presentation cycling through all 24 race/class combos. Sprites walk/roll/dash in from offscreen, perform attack animations, VFX plays at hit location (not on sprite), faction icon (top-left) and class icon (top-right) in circular frames, large LifeCraft Warcraft-font title centered, lore quote underneath, chat bubble with unique per-combo slogan. Factions: Crusade (Human, Barbarian → Odin), Legion (Orc, Undead → Madra), Fabled (Elf, Dwarf → The Omni). Data: FACTION_MAP, CLASS_ICON_MAP, HERO_SLOGANS (24 entries), LORE_QUOTES.
