...
- **Game Frame & Layout:** The application is wrapped in a `.game-frame` class with an ornate fantasy border frame overlay (`game-border-frame.png`) rendered as a `::after` pseudo-element at `z-index: var(--z-ui-frame, 10500)` with `pointer-events: none` for click passthrough. The frame image has a transparent center (processed via Python threshold 240+). Frame dimensions: ~5% top border, ~26% bottom border, ~2-3% side borders. All screen content is padded to fit within the frame's visible content area.
- **Z-Index Layer System (`src/constants/layers.js`):** Centralized layer constants organized from furthest (back) to closest (front). All components import from this single file instead of using hardcoded z-index values. Layers in order: BACKGROUND (0) → TERRAIN/DAY_NIGHT (1-4) → MAP_OBJECTS/NODES (5-10) → MAP_INFO/HUD (12-16) → MAP_DEV (30-40) → MAP_POPUPS (50-100) → SCENE internals (5-50) → BATTLE internals (0-500) → BOSS_WALKUP (5-30) → CHAT_BUBBLES (9500) → LOADING/LOOT/HERO_CREATE (9998-9999) → SETTINGS (9990-9991) → ADMIN_GIZMO (9998+) → BOTTOM_BAR (10000) → BOTTOM_BAR_POPUPS (10100) → UI_FRAME (10500) → FRAME_MASK/EDITOR (10501-10502) → INTRO_CINEMATIC (10600) → TOOLTIP (99999, portaled to body). Legacy MAP_LAYERS compatibility object also exported for WorldMap.
